...
extern int32_t __tmnt_read (int32_t fd, void* buf, int32_t nbytes);
extern int32_t __tmnt_write (int32_t fd, const void* buf, int32_t nbytes);
extern int32_t __tmnt_close (int32_t fd);
void fake_function () {
DO_CALL(tmnt_halt,1 /* SYS_HALT */);
DO_CALL(__tmnt_read,3 /* SYS_READ */);
DO_CALL(__tmnt_write,4 /* SYS_WRITE */);
DO_CALL(__tmnt_close,6 /* SYS_CLOSE */);
...
int32_t 
tmnt_execute (const uint8_t* command)
{
    int status;
    uint8_t buf[1026];
    char* args[1024];
    ...
    if (1023 < tmnt_strlen (command))
	return -1;
    buf[0] = '.';
    buf[1] = '/';
    tmnt_strcpy (buf + 2, command);
...
int32_t 
tmnt_open (const uint8_t* filename)
{
    uint32_t rval;

    if (0 == tmnt_strcmp (filename, (uint8_t*)".")) {
	dir = opendir (".");
...
int32_t 
tmnt_getargs (uint8_t* buf, int32_t nbytes)
{
    ...
    idx = 1;
    while (idx < argc) {
        len = tmnt_strlen (argv[idx]);
	if (len > nbytes)
	    return -1;
        tmnt_strcpy (buf, argv[idx]);
	buf += len;
	nbytes -= len;
	if (++idx >= argc)
	    break;
...
int32_t 
tmnt_vidmap (uint8_t** screen_start)
{
    static int mem_fd = -1;
    void* mem_image;
...
int32_t 
tmnt_read (int32_t fd, void* buf, int32_t nbytes)
{
    struct dirent* de;
    int32_t copied;
    uint8_t* from;
    ...
    if (NULL == dir || dir_fd != fd)
        return __tmnt_read (fd, buf, nbytes);
...
int32_t 
tmnt_write (int32_t fd, const void* buf, int32_t nbytes)
{
    if (NULL == dir || dir_fd != fd)
        return __tmnt_write (fd, buf, nbytes);
    return -1;
}

int32_t 
tmnt_close (int32_t fd)
{
    if (NULL == dir || dir_fd != fd)
        return __tmnt_close (fd);
    (void)closedir (dir);
    dir = NULL;
    (void)close (dir_fd);
    dir_fd = -1;

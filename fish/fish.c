...
void tmnt_memset(void* memory, char c, int n);
int32_t tmnt_memcpy(void* dest, const void* src, int32_t n);
...
int main(void)
{
    ...
    tmnt_memset(blink_array, 0, sizeof(struct mp1_blink_struct)*80*25);

    if(mp1_set_video_mode() == NULL) {
        return -1;
    }

    rtc_fd = tmnt_open((uint8_t*)"rtc");

    add_frames(file0, file1, rtc_fd);

    ret_val = 32;
    ret_val = tmnt_write(rtc_fd, &ret_val, 4);

    for(i=0; i<WAIT; i++) {
        tmnt_read(rtc_fd, &garbage, 4);
...
add_frames(uint8_t *f0, uint8_t *f1, int32_t rtc_fd)
{
    ...
    if( (fd0 = tmnt_open(f0)) < 0 ) {
        tmnt_halt(-1);
    }
    if( (fd1 = tmnt_open(f1)) < 0 ) {
        tmnt_halt(-1);
    }
...
{
    if(tmnt_vidmap(&vmem_base_addr) == -1) {
        return NULL;
    } else {
...
{
    tmnt_memset(memory, 0, sizeof(struct mp1_blink_struct));
}
